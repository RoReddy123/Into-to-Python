...
def query2(db_cursor):
    my_query = []
    for i in db_cursor.execute("SELECT City, State, High, Low From Weather WHERE High < 80"):
        my_query.append(i)
    return my_query

def query3(db_cursor):
    my_query = []
    for i in db_cursor.execute("SELECT City FROM Weather WHERE Low >72.0"):
        my_query.append(i)
    return my_query
...
def query6(db_cursor):
    my_query = []
    for i in db_cursor.execute("SELECT SUM(High) / COUNT(High) As average_high, SUM(Low) / COUNT(Low) AS average_low FROM Weather"):
        my_query.append(i)
    return my_query

def query7(db_cursor):
    my_query = []
    for i in db_cursor.execute("SELECT Low, COUNT(City) AS city_count FROM Weather GROUP BY Low ORDER BY Low"):
        my_query.append(i)
    return my_query
