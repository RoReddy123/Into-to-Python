...
def N(n_0, m, t):
    bacteria_speed = n_0*(math.exp((m*t)))
    return bacteria_speed

#INPUT t days
#RETURN number of teeth
def N_t(t):
     first = math.exp(-0.0685*t)
     second = math.exp(-8.96*first)
     third = math.ceil(71.8*(second))
     return third
...
def q(coef):
    ...
    if (a > 0):
        ...
        x = "down"
    x_coor = round(-b/(2*a),2)
    y_coor = round(a*(x_coor**2)+b*x_coor+c,2)
    y=(x_coor,y_coor)
    tuple = (x, y)
    return tuple
...
def RMS_mean(nlst):
    ...
    for i in nlst:
        expon += i**2
    return math.sqrt(expon/len(nlst))
...
def is_geo(xlst):
    ...
    for i in range(2,len(xlst)):
        if (xlst[i]/xlst[i-1] != xlst[1]/xlst[0]):
            return 0
    return 1
...
def track(start_pos, movement):
    ...
    end_pos = (x,y)
    count = countx+county
    displacement = round(net_displacement(start_pos, end_pos),2)
    return ((x,y),count,displacement)
    ...
    for team, score in game.items():
        if team in conference:
            if score > 0:
                ...
                conference[team]['L'] += 1
                conference[team]['PCT'] = conference[team]['W'] / (conference[team]['W'] + conference[team]['L'])
                conference[team]['Home'] += 1
        else:
            conference[team] = {'W': 1, 'L': 0, 'PCT': 1.0, 'Home': 1}
...
def go_fund_me(amt, donations):
    ...
    for i in range(len(donations)):
        total_donated += donations[i]
        if total_donated >= amt:
            remaining_donations = donations[i+1:]
            amount_over = total_donated - amt
            return (amt, remaining_donations, amount_over)
    amount_needed = amt - total_donated
    return (amt, [], -amount_needed)
...
def loan(cr, lst):
    ...
    for x in lst:
        countval = x[1]
        count = (countval["P"]*.35) + (countval["A"]*.3) + (countval["L"]*.15) + (countval["N"]*.1) + (countval["C"]*.1)
        if count > cr:
            dictlst += [[x[0],count]]
    return dictlst
